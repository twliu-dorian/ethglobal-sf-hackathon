...
const utils = {
    ...
    decToBinaryArray: (decimalString) => {
        const binaryString = BigInt(decimalString).toString(2).padStart(256, '0');
        const reversedBinaryArray = binaryString.split('');
    ...
    leftPadTo32Digits: (value) => {
        ...
        const paddedValue = valueString.padStart(totalLength, '0');
        return paddedValue;
    },
    generateProofJSON: (input) => {
    ...
    generatePublicJSON: (inputArray) => {
        const jsonString = JSON.stringify(inputArray, null, 2);
